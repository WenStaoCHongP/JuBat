...
@testset "Mode I monotonic bilinear" begin
    case, czm_mesh, meta, ms = _setup_strip()
    ms.damage_states = [JuBat.DamageState() for _ in 1:czm_mesh.n_cohesive]
    i = findfirst(e -> e.interface_type == :PE_PCC, czm_mesh.cohesive_elements)
    pe = case.param.PCC
    Λ = case.param.scale.L / case.param.scale.δ_czm
    n_steps = 40
    # 位移空间张开：扫过 δ_c
    u_max = 1.2 * pe.δ_c / Λ
    ...
    for s in 1:n_steps
        ...
        u, seps, tracts, ok, Rn, st = unit_czm_newton_step!(
            czm_mesh, u, case.param, ms.damage_states; bc_dofs=bc_dofs, bc_vals=bc_vals)
        ok && (ms.damage_states = st)
        @test ok
        δn, _ = seps[i]; Tn, _ = tracts[i]
        push!(δ_hist, δn)
        push!(D_hist, ms.damage_states[i].D)
        T_ana = analytic_bilinear_T(δn, pe.K_n, pe.δ_0, pe.δ_c)
        if δn < pe.δ_0 * 0.98
            @test Tn ≈ T_ana rtol=1e-2 atol=1e-8
        elseif δn < pe.δ_c
            @test Tn ≈ T_ana rtol=5e-2 atol=1e-6
        else
            @test abs(Tn) < 1e-3 * max(pe.K_n * pe.δ_0, 1.0)
            @test ms.damage_states[i].D > 0.99
        end
    end
    δ_max = maximum(δ_hist)
    @printf("[Mode I mono] δ_n: min=%.6e  max=%.6e  (δ_0=%.6e  δ_c=%.6e)  D_max=%.4f\n",
            minimum(δ_hist), δ_max, pe.δ_0, pe.δ_c, maximum(D_hist))
    @test δ_max > pe.δ_0
    @test δ_max > 0.5 * pe.δ_c   # 必须进入软化/近断裂，避免近零误通过
...
@testset "Mode I unload reload" begin
    ...
    Λ = case.param.scale.L / case.param.scale.δ_czm
    u = zeros(2 * czm_mesh.nnode)
    u_peak = 2.0 * pe.δ_0 / Λ   # D ≈ 0.5（δ_0 ≪ δ_c 时）
    local seps
    for s in 1:20
        u_n = u_peak * s / 20
        bc_dofs, bc_vals, _, _ = _bc_drive_cohesive(czm_mesh, i; u_n=u_n)
        u, seps, tracts, ok, _, st = unit_czm_newton_step!(
            czm_mesh, u, case.param, ms.damage_states; bc_dofs=bc_dofs, bc_vals=bc_vals)
        ok && (ms.damage_states = st)
        @test ok
    end
    D_peak = ms.damage_states[i].D
    δ_peak = seps[i][1]
    @printf("[Mode I unload] at peak: δ_n=%.6e  D=%.4f  (δ_0=%.6e)\n",
            δ_peak, D_peak, pe.δ_0)
    @test δ_peak > pe.δ_0          # 峰值必须越过损伤起始
    ...
    for s in 1:20
        ...
        ok && (ms.damage_states = st)
        @test ok
        @test ms.damage_states[i].D >= D_peak - 1e-12
    end
    D_unload = ms.damage_states[i].D
    ...
    for s in 1:10
        ...
        ok && (ms.damage_states = st)
        @test ok
        @test ms.damage_states[i].D ≈ D_unload rtol=1e-8
    end
end
