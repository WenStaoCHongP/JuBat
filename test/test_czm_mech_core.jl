...
# 2026-08-30 重构适配：装配函数直读 param；缓存经 czm_mesh 惰性访问器；
# damage_states 显式传入（不再挂网格）。
...
@testset "新入口与 K_bulk*u 逐位等价（线弹性槽位）" begin
    case = build_mech_core_fixture()
    czm_mesh = case.czm_mesh
    u = make_test_u(czm_mesh.nnode)

    K_ref = JuBat.assemble_bulk_stiffness(czm_mesh, case.param)
    f_ref = K_ref * u

    f_int, K_tan = JuBat.assemble_bulk_residual_tangent(czm_mesh, u, case.param)
...
@testset "零位移给零内力（线弹性无预应力）" begin
    ...
    f_int, K_tan = JuBat.assemble_bulk_residual_tangent(czm_mesh, u0, case.param)
...
@testset "缓存不变量：传入 K_bulk_cached 时直接复用同一对象" begin
    ...
    K_bulk = JuBat.bulk_stiffness(czm_mesh, case.param)
    f_int, K_tan = JuBat.assemble_bulk_residual_tangent(
        czm_mesh, u, case.param; K_bulk_cached=K_bulk)
...
@testset "切线对称性（线弹性 + 平面应力各向同性）" begin
    ...
    _, K_tan = JuBat.assemble_bulk_residual_tangent(czm_mesh, u, case.param)
...
@testset "未实现槽位与冲突参数必须报错，不得静默降级（AGENTS 9.7 / spec §6）" begin
    ...
    @test_throws ErrorException JuBat.assemble_bulk_residual_tangent(
        czm_mesh, u, case.param; geo_nl=true, K_bulk_cached=K_bulk)
    @test_throws ErrorException JuBat.assemble_bulk_residual_tangent(
        czm_mesh, u, case.param; plasticity=true)
    # mech_state 的消费者在 Batch 3 引入；此前传入非 nothing 即报错
    @test_throws ErrorException JuBat.assemble_bulk_residual_tangent(
        czm_mesh, u, case.param, :dummy_state)
end
...
@testset "位移向量长度不符必须报错，不得截断或补零" begin
    ...
    @test_throws DimensionMismatch JuBat.assemble_bulk_residual_tangent(
        czm_mesh, u_short, case.param)

    u_long = zeros(Float64, 2 * czm_mesh.nnode + 3)
    @test_throws DimensionMismatch JuBat.assemble_bulk_residual_tangent(
        czm_mesh, u_long, case.param)
end
...
@testset "assemble_coupled_system_full 残差构成不变" begin
    ...
    K_tot, R, F_tc, _, _ = JuBat.assemble_coupled_system_full(
        czm_mesh, u, case.param, dT_elem, Δsoc_n, Δsoc_p;
        damage_states=case.mech.damage_states,
        K_bulk_cached=JuBat.bulk_stiffness(czm_mesh, case.param))
