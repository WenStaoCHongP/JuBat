...
**注 3.3.1（$D$ 的物理诠释）**：$D$ 表征界面面积的"损伤分数"——$D=0$ 对应全部面积有效粘结，$D=1$ 对应全部面积脱粘。这一诠释与 §2.8.2 式 (2.68) $A_{\mathrm{eff}}=1-D$（无量纲面积分数，$A_0\equiv1$ 归一）一致：$D$ 等于脱粘面积分数。
...
脱粘导致电化学反应面积丧失：
- 完好界面：活性面积 $A_0$；
- 部分脱粘：有效反应面积分数 $A_{\mathrm{eff}}(D)=1-D$（无量纲，绝对面积 $A_0\,A_{\mathrm{eff}}$）；
...
| 反应面积 | $A_{\mathrm{eff}}(D)=1-D$（式 3.102，$A_0\equiv1$ 归一） | $-1$（式 3.104） | 无量纲 | 电流密度修正（§3.6.2 应用位置） |
