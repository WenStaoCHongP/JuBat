...
const OUTPUT_DIR = joinpath(@__DIR__, "..", "output", "change_model")
mkpath(OUTPUT_DIR)
...
for i = 1:3
    ...
    result1 = CSV.read(path * "pybamm_" * models[i] * "_1C.csv", DataFrame, header = 1)
    pybamm_1C = Matrix(result1)
...
savefig(joinpath(OUTPUT_DIR, "change_model.pdf"))