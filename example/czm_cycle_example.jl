...
- 二维分布式热模型
- 内聚力损伤模型（层间脱粘）
- 充放电循环：充电3600s → 静置600s → 放电3600s → 静置600s
- 损伤跨循环累积
...
function main()
    ...
    cycle_opt = JuBat.CycleOption(
        n_cycles = 10,           # 演示用5次循环，实际可设50次
        t_discharge = 1800.0,      # 充电时长 (s)
        t_rest1 = 1000.0,        # 充电后静置 (s)
        t_charge = 1800.0,   # 放电时长 (s)
        t_rest2 = 1000.0,        # 放电后静置 (s)
        I_discharge = 5.0,      # 放电电流 (A) - 1C
        I_charge = 5.0,         # 充电电流 (A) - 1C
        V_lower = 2.5,          # 放电截止电压
        V_upper = 4.2,          # 充电截止电压
        SOC_init = 0.65,         # 初始SOC设为65%（避免过高初始SOC导致的异常库伦效率）
    ...
    total_time = cycle_opt.n_cycles * (cycle_opt.t_charge + cycle_opt.t_rest1 + cycle_opt.t_discharge + cycle_opt.t_rest2)
    @printf("  总仿真时间: %.1f小时\n", total_time / 3600.0)
