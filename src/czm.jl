...
"""
function _find_coincident_node_pairs(mesh::Mesh, inner_nodes::Vector{Int64}, outer_nodes::Vector{Int64}, tol::Float64)
...
"""
function _create_cohesive_elements_direct(mesh::Mesh, interface_pairs::Vector{Tuple{Int64, Int64}})
    ...
    # 按角度排序节点对
    sorted_pairs = sort(interface_pairs, by = p -> atan(mesh.node[p[1], 2], mesh.node[p[1], 1]))
...
```
"""
function _create_cohesive_elements_from_pairs(node::Matrix{Float64}, interface_info::Vector{InterfaceNodeGroup})
    cohesive_elements = CohesiveElement[]
    elem_id = 0
...
- `D`: 当前损伤变量
"""
function bilinear_traction(δ_n::Float64, δ_t::Float64, damage_state::DamageState, cohesive_params::Cohesive; update::Bool=false)
    # 提取参数
...
- `cohesive_params`: 内聚力参数
"""
function update_damage!(damage_states::Vector{DamageState}, separations::Vector{Tuple{Float64, Float64}},cohesive_params::Cohesive)
...
- `T_n, T_t`: 牵引力
"""
function cohesive_element_matrices(elem::CohesiveElement, node::Matrix{Float64},u::Vector{Float64}, damage_state::DamageState,cohesive_params::Cohesive)
...
- `tractions`: 每个单元的牵引力
"""
function assemble_czm_system(czm_mesh::CohesiveMesh, u::Vector{Float64}, cohesive_params::Cohesive)
...
"""
    assemble_thermal_chemical_load(czm_mesh, E_eff, ν_eff, α_eff, β_n, β_p, dT_elem, Δsoc_n_elem, Δsoc_p_elem)
...
"""
    assemble_coupled_system(czm_mesh, u, E_eff, ν_eff, cohesive_params;F_ext=nothing, F_thermo_chem=nothing)
...
- `tractions`: 牵引力
"""
function assemble_coupled_system(czm_mesh::CohesiveMesh, u::Vector{Float64},E_eff::Float64, ν_eff::Float64, cohesive_params::Cohesive;F_ext::Union{Vector{Float64}, Nothing}=nothing,F_thermo_chem::Union{Vector{Float64}, Nothing}=nothing)
...
"""
    assemble_coupled_system_full(czm_mesh, u, E_eff, ν_eff, α_eff, β_n, β_p,cohesive_params, dT_elem, Δsoc_n_elem, Δsoc_p_elem;F_ext=nothing)
...
"""
function assemble_coupled_system_full(czm_mesh::CohesiveMesh, u::Vector{Float64},E_eff::Float64, ν_eff::Float64,α_eff::Float64, β_n::Float64, β_p::Float64,cohesive_params::Cohesive,dT_elem::Vector{Float64},Δsoc_n_elem::Vector{Float64},Δsoc_p_elem::Vector{Float64};F_ext::Union{Vector{Float64}, Nothing}=nothing)
    ndof = 2 * czm_mesh.nnode
    
    # 组装基本系统
    K_total, f_int_total, separations, tractions = assemble_coupled_system(czm_mesh, u, E_eff, ν_eff, cohesive_params)
...
"""
function apply_bc_czm!(K::SparseMatrixCSC{Float64,Int64}, F::Vector{Float64},bc_nodes::Dict{Int64, Symbol})
...
- `bc_vals`: 约束值
"""
function apply_bc_czm!(K::SparseMatrixCSC{Float64,Int64}, f::Vector{Float64},bc_dofs::Vector{Int64}, bc_vals::Vector{Float64})
...
"""
    newton_raphson_czm(czm_mesh, F_ext, E_eff, ν_eff, cohesive_params, param_dim;α_eff=0.0, β_n=0.0, β_p=0.0,dT_elem=nothing, Δsoc_n_elem=nothing, Δsoc_p_elem=nothing,max_iter=50, tol=1e-8, u0=nothing)
...
function newton_raphson_czm(czm_mesh::CohesiveMesh, F_ext::Vector{Float64},E_eff::Float64, ν_eff::Float64,cohesive_params::Cohesive, param_dim;α_eff::Float64=0.0, β_n::Float64=0.0, β_p::Float64=0.0,dT_elem::Union{Vector{Float64}, Nothing}=nothing,Δsoc_n_elem::Union{Vector{Float64}, Nothing}=nothing,Δsoc_p_elem::Union{Vector{Float64}, Nothing}=nothing,max_iter::Int=50, tol::Float64=1e-8,u0::Union{Vector{Float64},Nothing}=nothing)
    ...
    has_thermo_chem = (α_eff != 0.0 || β_n != 0.0 || β_p != 0.0) && dT_elem !== nothing && Δsoc_n_elem !== nothing && Δsoc_p_elem !== nothing
...
"""
    solve_czm_step(czm_mesh, F_ext, E_eff, ν_eff, cohesive_params, param_dim, u_prev;α_eff=0.0, β_n=0.0, β_p=0.0,dT_elem=nothing, Δsoc_n_elem=nothing, Δsoc_p_elem=nothing,max_iter=50, tol=1e-8)
...
- `result`: CZMResult
"""
function solve_czm_step(czm_mesh::CohesiveMesh, F_ext::Vector{Float64},E_eff::Float64, ν_eff::Float64,cohesive_params::Cohesive, param_dim,u_prev::Vector{Float64};α_eff::Float64=0.0, β_n::Float64=0.0, β_p::Float64=0.0,dT_elem::Union{Vector{Float64}, Nothing}=nothing,Δsoc_n_elem::Union{Vector{Float64}, Nothing}=nothing,Δsoc_p_elem::Union{Vector{Float64}, Nothing}=nothing,max_iter::Int=50, tol::Float64=1e-8)
    
    # 使用上一步位移作为初始猜测
    result = newton_raphson_czm(czm_mesh, F_ext, E_eff, ν_eff, cohesive_params, param_dim;α_eff=α_eff, β_n=β_n, β_p=β_p,dT_elem=dT_elem, Δsoc_n_elem=Δsoc_n_elem, Δsoc_p_elem=Δsoc_p_elem,max_iter=max_iter, tol=tol, u0=u_prev)
