...
"""
    solve_phase(case, phase_type, t_max, I_current, V_limit, initial_state;czm_mesh=nothing, czm_params=nothing)
...
function solve_phase(case::Case, phase_type::PhaseType, t_max::Float64, I_current::Float64, V_limit::Float64,initial_state::Dict;czm_mesh=nothing, czm_params=nothing,dt_range::Vector{Float64}=[1.0, 10.0])
    ...
    V_init = get(initial_state, "V", nothing)
    ...
    result.V_start = phase_result_data["V_start"]
...
"""
    _solve_phase_internal(case, phase_type, t_max, I_current, V_limit, y0, T_nodes, czm_mesh, czm_params, dt_range)
...
- 颗粒内锂浓度会因扩散而趋向均匀化
"""
function _solve_phase_internal(case::Case, phase_type::PhaseType, t_max::Float64, I_current::Float64, V_limit::Float64, y0, T_nodes, czm_mesh, czm_params, dt_range)
    # 静置阶段特殊处理：确保电流为0
