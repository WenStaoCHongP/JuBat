...
include("Czm.jl")                # CZM 本构与系统装配（实际文件名 czm.jl）
...
# 5. 几何、力学与扩展
include("mechanical.jl")         # 实际文件名 Mechanical.jl
...
> **已知问题**：include 写的是 `"Czm.jl"` / `"mechanical.jl"`，实际文件名为
> `czm.jl` / `Mechanical.jl`（大小写不一致，Windows/macOS 可运行，Linux 会失败），
> 建议后续统一。
...
> **已知问题**：`export Postprocessing`（实际函数名为 `PostProcessing`，导出名无对应
> 定义）与 `export ThermalModel, ThermalLumpedModel, ThermalDistributed2DModel`
> （src 中无同名定义）为无效导出，待修。
