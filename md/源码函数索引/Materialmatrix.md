...
| L121 | `if δ_eff > δ_max_hist; if δ_eff <= δ_0_eff; ...; elseif δ_eff >= δ_c_eff; ...; else; ...; end; ...; end`（嵌套 2 层 + 多个 `elseif`，跨 L121-L140） | 抽出 `compute_D_eq(δ_eff, δ_0_eff, δ_c_eff) -> Float64` 纯函数；该逻辑在 `bilinear_traction_state` (L121-L140) 与 `bilinear_tangent` (L225-L231) 重复 |
| L225 | `if is_loading && δ_eff > δ_0_eff && δ_eff < δ_c_eff; ...; elseif is_loading && δ_eff >= δ_c_eff; ...; elseif is_loading && δ_eff <= δ_0_eff; ...; end`（多个 `&&` 链，跨 L225-L231） | 与 L121-L140 同逻辑的独立实现，建议共用 helper |
| L258 | mixed-mode 切线刚度的嵌套 `if δ_eff ≤ δ_0_eff \|\| !is_loading; if δ_n ≥ 0; ...; else; ...; end; elseif δ_eff ≥ δ_c_eff; ...; else; if δ_n ≥ 0 && δ_eff > 1e-15; ...; else; ...; end; end`（嵌套 3 层 + 多条件，跨 L258-L282） | 抽出每段独立分支函数 `tangent_elastic / tangent_fractured / tangent_softening`，主流程分派 |
