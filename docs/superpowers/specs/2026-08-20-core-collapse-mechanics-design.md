...
| D6 | 双文档：本规格在 `docs/superpowers/specs/`，任务三件套在 `docs/planning-with-files/30_堆芯塌陷力学建模/` |
...
**新增确定发现**（证据详见 `docs/planning-with-files/30_堆芯塌陷力学建模/findings.md` 第二轮复核节 E1–E13）：理论几何参数表与代码逐项不符（疑因把 21700 直径当半径，L_turn 3.5×、L_spiral 7.8×、匝数 2.2×）；`(1−D)⁻²` 由"疑似"升级为确定的内部矛盾且支撑文献不可核查；每层厚度方向仅 1 个 Q4 导致单层起皱不可解；`07` 同号小节矛盾、`K_uu` 双定义、弧长法缺失、Φ 约束未入弱形式、`κ_ss` 与 C⁰ Q4 不兼容、层编号双约定冲突、附录 A/C 多处脱节、`Theory/审查/` 悬空链接。
