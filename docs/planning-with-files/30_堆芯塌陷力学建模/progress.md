# 堆芯塌陷力学建模：进度记录

## 会话：2026-08-17

### 计划制定（brainstorming，architectural 路径）

- **状态：** complete
- 三个并行探索完成：界面术语统一任务契约、力学/CZM 代码现状、Theory 工况 C 验收要求（C1–C5/L1–L4）。
- 用户四项决策确认：全路线分批、czm 开关为主开关内部子选项、operator-splitting 先行、双文档。
- 计划自评审发现并修正：P0 本构缺参报错须限定工况 C 路径、连续反馈必须 opt-in、参数集扩展遗漏、CycleSolver 遗漏、记录精度判据。
- 用户两次修订：①各向异性=逐材料各向同性+叠加等效，修复理论错误；②SP–电极先完美接触，接触/摩擦后置 Batch 8。

### Batch 0'：Theory 各向异性表述修复

- **状态：** complete
- 修订仅涉及 `Theory/03_本构理论.md`（详见 `findings.md` 修订表）：§2.1 表格与注 2.1b、统一二维约化、注 2/注 2.3、§2.3.5 输出、§2.4.2 CLT Q̄、§2.4.4 整节重写（公式 2.35–2.40 重排单调）、§2.4.5、§2.6（含 ε_zz 各向同性闭合 (2.42a)/(2.47)）、§2.6.4 输出与自查门槛。
- 联动核查 Theory/00/02/04/05/06/07/08/09/10/CLAUDE：均无需改动。
- 建立本目录三件套并更新 `docs/planning-with-files/index.md`（29 目录、93 文件）。

## 会话：2026-08-20

### 理论验证 + FEM 可行性评审 + spec v1.1 修订

- **状态：** complete（spec 待用户评审）
- 通读 Theory 00/02/03/04/07/08 工况 C 相关段落 + 代码锚点核验（Czm/CzmSolve/CouplingState/Jellyrollmodel）。
- 发现 5 项理论缺陷（Δ_core 基线污染、(1.30a) 坐标系错配、R-EC-1 疑似双计、Q_contact j 语义、J2 表述），详见 findings。
- FEM 可行性确认：全部标准算法，风险在网格长宽比与 C4-lite 可达性，非算法本身。
- 用户 4 项决策（D8–D11）确认并写入 spec：位移基 Δ_core、完全 GL TL、探针后停止、预应力 opt-in Batch 2'。
- spec 更新：头部 v1.1 行、决策表 D8–D11、§3.2/3.3/3.4/3.5/3.6 修订、新增 §3.7、§4.1/§5/§7/§8/§9 联动、§10.1 评审记录；自评审补齐 §1↔§8 SP 热膨胀交叉引用。

### 第二轮独立复核（全 13 文件 + 代码交叉核验）

- **状态：** complete（待用户就 4 项存疑决策商讨）
- 4 个并行子代理分片复核（01/03、05/06、07/08、09-11），逐条重读原文与代码独立验证；子代理若干条经核查为表述不准或与独立判断不符，未采纳。
- **更正第一轮两处错误结论**：`Q_contact` 中 `j` 是界面电流密度（A/m²）而非分支总电流，且体积源需乘 `a_s`；`t_repeat` 代码实参为 373.6 μm 而非理论表的 284 μm。
- 新增确定发现 E1–E13，其中影响决策的三项：E1 理论几何参数表与代码不符（L_turn 3.5×、L_spiral 7.8×、匝数 2.2×，疑因把直径当半径）；E2 `(1−D)⁻²` 由「疑似双计」升级为**确定内部矛盾**（§3.6.1 几何推导 vs §4.5.2 材料劣化辩护互斥，支撑文献不可核查）；E13 每层厚度方向仅 1 个 Q4、薄层长宽比 29–38:1 ⟹ 单层起皱不可解、叠层椭圆化可解。
- 用户 4 项决策 D12–D15 确认并写入 spec v1.2：`(1−D)⁻²` 延后至 Batch 6 专项推导、网格探针后定能力边界、几何表全库重算 + 字段溯源、理论修订拆 Batch 0''/Batch 7。
- spec v1.2 更新：头部版本行、决策表 D12–D15、§3.5 数值更正、§3.6 `j`/`a_s` 更正与 D12 三候选、新增 §3.8 网格能力边界与探针、§4.1 新增探针脚本与 Theory 行、§7 新增 Batch 0''/2'' 行、§8 风险表四项重写、§9 批次顺序插入 0''/2'' 并列出 8+5 项清单、§10.2 复核记录。

## 验证记录

| 检查 | 预期 | 当前结果 | 状态 |
|---|---|---|---|
| grep `横观各向同性` | 仅注 2.1b 废除声明 | 符合 | 通过 |
| grep `正交各向异性` | 仅叠层等效矩阵形式描述 | 符合 | 通过 |
| grep `E_s^i/G_{sn}^i/ν_{sz}/ν_{nz}` | 0 | 0 | 通过 |
| 公式 tag 顺序 | 2.35→2.40 单调 | 符合 | 通过 |
| 尾随空白 | 0 | 0 | 通过 |
| 全套测试 | 与既有状态一致 | 26 通过/1 失败（`unit_czm_eigenstrain.jl` 既有缺口） | 通过 |
| Git 变更范围 | 仅 Theory/03 + 本目录 + 索引 | 符合 | 通过 |

## 错误日志

| 时间 | 错误 | 尝试 | 处理 |
|---|---:|---:|---|
| 2026-08-17 | ugrep 对混合中文+LaTeX 花括号模式报 invalid character class | 2 | 拆分简单模式分别检索 |
| 2026-08-17 | Git Bash 下 sed/grep 输出中文文件名乱码 | 多次 | 行号仍有效；内容核对改用 Read 工具 |

## 会话：2026-08-21

### 实现计划评审 + spec v1.3 / 计划 v1.1 修订（方案 B）

- **状态：** complete（Batch 0'' 待用户放行）
- 评审 Batch 0''+1 实现计划（`49ec452`）并对 HEAD `e117fd2` 逐项核实：3 处前提失效（探针被删/PNG 路径迁移/eigenstrain 已修复）+ 4 项次要缺口，详见 findings"实现计划评审"节；核对无误项（行号、签名、几何数值、基线冻结值）未改动。
- 用户决策**方案 B**：Batch 1 基线门禁改用 `tools/verify_czm_standalone.jl`；spec v1.3（§头部/§5/§7/§10.3）与计划 v1.1（Task 1 重写为复核、Task 2 KKT 扩清理、Task 3 计数订正、测试预期 22/22 全绿、PNG 路径更正、偏差登记第三项、修订记录 8 条）同步修订。
- **基线已冻结**：实测运行 `verify_czm_standalone.jl`（exit 0），快照写入 `baseline_czm_standalone.md`——网格 10946 节点/6728 bulk/3364 cohesive；8×3 收敛表（0.1–5.0 全 OK，10.0 三方法 FAIL 冻结）；Summary 三方法各 7/8、total_iter 16/794/2187。工具既有瑕疵两条登记 findings，不修改。
- 待提交变更：spec v1.3、计划 v1.1、`baseline_czm_standalone.md`、findings/progress/index（提交须经用户授权绕过 Mimosa 守卫）。

### 会话：2026-08-21（续）：Batch 0''+1 计划执行

#### Task 1：基线复核

- **状态：** complete
- 重跑 `verify_czm_standalone.jl`（HEAD `40757d7`，与冻结同环境），全部数据行与冻结快照**逐位一致**（含 10.0 水平 FAIL 与 stall 警告）——基线可复现，Batch 1 门禁有效。
- findings 的工具瑕疵登记在位（`:66`/`:134` 传参不一致、未合并 thermal2D）。

#### Task 2–4：Batch 0'' 理论修订（8 项阻塞矛盾）

- **状态：** complete
- ① `07` 同号 §6.4.5 去重（保留位移阈值代数更新版）+ `:573` KKT 引用清除；执行中另清理 5 处现行 KKT 表述（§1.1 表 ×2、§6.9 输入 3、§6.9.6 校核表、本章小结）。
- ② `K_uu` 统一三项定义（式 6.8 补 cohesive 项、`K_G(σ)→K_G(S)`；6.54 改复述）；`:1073` 引用错位转 Batch 7。
- ③ 新增 §6.10 柱面弧长法（Crisfield 式 6.88–6.91，λ 缩放本征应变增量）。
- ④ §2.6 后补 Φ 跨匝配对施加方式（节点合并/主从消元，非弱形式项）。
- ⑤ `02` 式 (1.30a) 后加 κ_ss 与 C⁰ Q4 不兼容实现注记（D9，含能力边界：单层起皱不可解）。
- ⑥ `01` 插入层编号双约定声明（5 材料类型 vs 8 物理层序）。
- ⑦ `A_eff` 全库统一无量纲（计划 3 处 + 执行发现 7 处；`09:62` R_contact 单位 Ω→Ω·m²；D12 冻结区未触碰）。
- ⑧ 几何参数全库按代码重算（21.03 匝 / 0.7727 m / 373.6 μm / γ∈[0.3485°,1.7744°]，02 表新增"代码字段"列；计划外同源值 6 组一并重算，见 findings）。
- **门禁**：`rg "46.6|0.132"` 0 命中；`rg "284"` 仅 `06:69` ρ≈2846 豁免；派生比值 0 残留；层编号声明/A_eff/κ_ss 三 grep 通过。
- **全套测试**：`test/runtests.jl` 22/22 通过（3m37.6s，含 eigenstrain 60/60），文档批次零回归。
- 提交：Task 2 `d28b3b4`、Task 3 `879440c`、Task 4（本批）。

#### Task 5–7：Batch 1（Option 子选项 + bulk 残差/切线统一入口 + 接线）

- **状态：** complete
- Task 5（`cd7be6e`）：`Option` 新增 6 个默认关子选项（`czm_geo_nonlinear`/`czm_winding_prestress`/`czm_j2_plasticity`/`czm_phi_bond`/`czm_continuous_feedback`/`czm_friction_mu`），TDD 三 testset 锁定默认值契约。
- Task 6（`016a8b9`）：`src/czm.jl` 新增 `assemble_bulk_residual_tangent`（线弹性槽位：`f=K_bulk*u`、`K_tan=K_bulk`；`geo_nl`/`plasticity`/`mech_state` 非默认即 `error`；`K_bulk_cached` 透传同一对象）。6/6 testset 通过，含与 `K_bulk*u` 逐位等价、缓存同一对象（`===`）、对称性、维度/槽位报错。
- Task 7：`assemble_coupled_system` 改走新入口（`assemble_coupled_system_full` 间接经它）。与 spec §4.1/§4.2 偏差三项已登记（Option 提前全加、`PlasticState`/`MechHistory` 延后、`K_bulk_cached` 签名扩充）。
- **三道门禁实测**：全套 `runtests.jl` **24/24**（4m09s，含两个新测试文件）；`verify_czm_standalone.jl` 快照与冻结基线**逐位一致**（8×3 表含 10.0 FAIL 条目与 Summary）；`testexample.jl` exit 0，网格 1682/1763、步数 19、电压 4.0367/3.9438/0.0929、容量 0.0833 Ah、温度 298.15–299.00 K、D 0.0000%、分离 1.2557e-14 m、断裂 0，全部与冻结表一致，PNG SHA-256 `4ba6207c…e932` 一致。
- **A/B 复核**：冻结表"CZM converged updates 19/19"不对应脚本打印行（日志实为 18 条 `converged=true` debug 行）；经 stash A/B 确认**接线前后同为 18 行、同一 PNG SHA**，属登记口径而非本批漂移；已在本节记录，不改冻结表（该行非脚本打印指标）。
- `Simplify/baseline.md` 批次表已追加 Batch 1 行（PASS）。

### 会话：2026-08-21（续二）：Batch 2（K_G/C1）执行

- **状态：** complete
- 计划评审通过（用户批准 D-B2-1/D-B2-2，计划 `2026-08-21-core-collapse-batch2-plan.md`，提交 `a4b58a4`）。
- Task 1（`7cc8b0d`）：`gl_element_residual_tangent`（完全 GL、SVK、标准初应力 K_G）+ `geo_nl` 槽位启用 + `eigenstrain` 关键字；5/5 testset（FD 切线、30° 刚体转动机器零内力、u=0 切线≡线性刚度 rtol 1e-12、自由膨胀零应力、K_G 压缩方向性）。
- Task 2（`aea1e85`）：mech_core 夹具改传 `case.param`（归一化网格对齐生产）；Batch 1 的"geo_nl 未实现须报错"断言随槽位实现演进为"缓存冲突契约"断言。
- Task 3（`b551dac`）：`geo_nl`/`eigenstrain` 贯穿 `assemble_coupled_system` → basic/load_substep（含 backtrack 与 newton 线搜索）→ `update_czm_damage!` 生产调用点；`arc_length+geo_nl` 显式 error（Batch 5）；C1 2/2（K_G→0 极限 geo≈线性 rtol 1e-4；geo off 与 Batch 1 逐位一致）。
- **门禁执行中发现并修复**：`b551dac` 的 4 空格缩进 replace_all 以子串方式误伤嵌套 arc 函数内同文本行（`eig_kwargs` 未定义 → 探针 arc 方法 exit 1）——**Gate B 门禁捕获**，回滚 `1bb0026`，重跑探针与冻结基线逐位一致。教训：对"缩进即作用域"的嵌套函数禁用短前缀 replace_all。
- **三道门禁全绿**：全套 **26/26**（4m21s）；`verify_czm_standalone` 快照逐位一致；testexample 全部冻结指标与 PNG SHA `4ba6207c…e932` 一致（geo_nl 默认关零漂移）。
- 执行偏差（计划小瑕疵，现场修正）：① FD 测试 dof=5000 超出 nθ=8 网格自由度 → 改动态取；② isapprox 自定义 norm 单参调用 MethodError → 改稠密比较；③ Batch 1 geo_nl 报错断言过期（上文 Task 2）。

### 会话：2026-08-21（续三）：Batch 3（J2 塑性/C2）执行

- **状态：** complete
- 计划 `2026-08-21-core-collapse-batch3-plan.md`（`b52c4ba`，用户指示直接执行）；参数联网核实（findings"Batch 3 参数核实"节：Al 15μm 硬态 UTS 150–290 MPa/屈服≈0.9UTS、ED Cu 108–441 MPa、薄硬态箔硬化近耗尽——spec 缺省 60/200/0 落点合理）。
- Task 1（`4fcb243`）：`src/CzmPlasticity.jl`——`PlasticState`、`foil_params_of`、4×4 Newton 平面应力 J2 一致返回映射（切线由收敛系统线性化精确导出）、物理箔参数（PCC 70GPa/0.33/60MPa、NCC 110GPa/0.34/200MPa、H=0，σ_czm 归一）；`unit_czm_j2.jl` 5/5（弹性不变/屈服面停留/卸载永久应变/一致切线 FD/组合回面）。执行修正：`J[4,1:3].=n'` 广播维错、`[I zeros]` 拼接错、单轴 σ11=σ_y 断言在平面应力关联流动（n=[1,−0.5,0]）下不成立改为不变量断言。
- Task 2（`495287c`）：GL 单元 plastic/commit_to、装配 mech_state 位置参数、求解器全链路、`CzmLayout.plastic_states` 持久化、收敛后 `commit_plastic!`；集成 4/4（C2-lite：Δsoc=−0.3 失配驱动下 PCC/NCC κ>0 其余层 0、回归锚、不可逆有界、非法组合报错）。执行修正三处：mech_state 误作关键字、提交元组缺嵌套、**fill(PlasticState(),ne,4) 可变对象别名共享**（经典陷阱，测试与生产初始化都已改独立构造）。
- **语义说明（D-B3-2 推论）**：同载荷重解存在有界的再预测漂移（κ 二次增长 ~1.6×），单调不可逆性保持；生产时序每步新 Δε* 下漂移为二阶小量。
- **三道门禁全绿**：全套 28/28（5m07s）；探针快照逐位；testexample 冻结指标与 PNG SHA 一致（塑性默认关零漂移）。

### 会话：2026-08-22：Batch 2'（卷绕预应力）执行 + 用户参数修正与基线重冻结 v2

- **状态：** complete
- 计划（`c8d9a4b`，用户批准 D-B2'-1 等应变卷入张力）；Task 1（`4d611ba`）σ₀ 场 + 参数（T_ne=3 MPa/T_pe=1 MPa，§10.4.1）+ 解析对照 40 单元互核、量级、缺参拦截。
- Task 2（`1a74411`）：σ₀ 叠加进 GL 残差与 K_G（D-B2'-3 零值旁路）、prestress 全链路、CzmLayout 持久化、CallModel 标记；预应力 6/6。
- **用户参数修正（同批）**：PCC.E→70 GPa/0.33、NCC.E→110 GPa/0.34、E_coat→1 GPa、SP.E→750 MPa/0.35；按指示移除 E_foil/nu_foil 冗余字段，塑性 D 统一走 moduli_of（**D-B3-0 由参数修正取代**——注意 param.PCC.E 为 ÷E_coat 归一，须走 ×E_coat/σ_czm 链，直取会差 6.1 倍）。
- **执行发现**：① 等应变分担下刚性箔 σ_foil = T·E_f/Ē ≈ 41 MPa（放大 ~14×，物理正确）；② σ₀ 场为混合结构——箔全拉、内层软涂层受压（其卷入张力 ~0.4 MPa < 累积压力 ~3.2 MPa）；③ 首平衡 basic 8 步收敛，load_substep 软收敛 ~7e-8（低于子步容差 1e-7、高于严格 tol——线搜索特性非缺陷，切线 FD 1e-12 已证一致）。
- **基线重冻结 v2（按既定程序）**：参数刻意变更 → 探针景观移动（basic 8/8 且 D=0.55 出现损伤；load_substep/arc_length 转 6/8）与 testexample 两项力学输出移动（分离 ×10、PNG SHA `b31ffb49…`）；电化学/热指标全部不变；v1 档案保留。**后续批次的冻结门禁以 v2 为准。**
- **门禁**：全套 29/29（5m29s）；探针与 testexample 按 v2 重冻结值逐位一致。

### 会话：2026-08-22（续）：Batch 2''（D13 网格探针）执行

- **状态：** complete
- `5c9bc52`（T1）：厚涂层（PE/NE）径向细分 thin_subdiv + split_KG 切线分离（默认零漂移 4/4 + 回归组零失败；执行中修复三处编辑误伤，线性路径 0 失败复核；build_thermal_to_czm_interp 螺旋数下限泛化）。
- `eaec2b0`（T2）：探针脚本。冒烟发现并修复两处物理缺陷：①split 只含 bulk 须补 cohesive 切线（否则层间解耦出机构、K0 非正定）；②模态 DFT 须在第一匝窗口（内螺旋跨 21 匝无整螺旋周期性）。
- **D13 判定（spec v1.4 §3.8.1）**：模态稳定（n=1 叠层级，优势 30–161×，无单层起皱型模态）；μ_crit 不稳定（nθ 3.4×、细分 2.3×）→ 定量宣称需单元技术改造批次；定性边界冻结"叠层级不圆度"。预应力参考态近临界（μ≈5e-3）。
- **三道门禁（v2）**：全套 30/30（5m39s）；探针 Summary 三行与 v2 逐位一致；testexample 指标与 PNG SHA `b31ffb49…` 一致。

### 会话：2026-08-22（续二）：Batch 5 执行至 Task 2（安全点）

- **状态：** Task 1 complete；Task 2 WIP（阻塞已记录）；Task 3（C4-lite + v3 重冻结）待执行
- T1 `8ab8863`：Φ 缝默认完美粘结——双网格方案（CzmSubmesh 增 mesh_bonded/phi_keep；**插值 mod 定位与行数契约零改动**——用户否决坐标法/闭式后定稿）；开关字段删除。执行中三次方案迭代（子网格直接合并→CohesiveMesh 层合并→双网格），前两次失败原因记 findings。
- T1 `238e71a`：Δ_core（core_ovalization：单匝窗口由 winding_turn 反推——element[1,2] 是跨全部匝的螺旋总节点数；天然角序禁 atan 排序）+ CzmLayout.node_ref 持久快照；测试 3/3。
- T2 `aa69304`：Crisfield geo 弧长列式与路由完成（f̂ 符号/BC 置零/Δl 解尺度/λ=0 死循环/子步 10tol/根守卫六个缺陷逐一修复），**收敛受切线近奇异模态阻塞**（findings 详录；@test_broken 登记，解除条件 = D13 ② 或切线预条件）。全套 33/33。
- **T3 待办**：C4-lite 多圈集成（用 load_substep 推进，不用弧长）→ 探针/testexample v3 重冻结（数据采集中）→ 记录提交。

### 会话：2026-08-23：Batch 5 T3 执行（探针 v3 / testexample v3 / C4-lite D10）

- **状态：** T1/T2 见上节；本节 T3 核心结论已落，收尾门禁未完（用户停止）
- **"nθ=40 发散缺陷"撤销（`c8aea63`）**：证伪链——①早前发散测试误用 α_eff=1.0/dT=0.1（10% 本征应变，未合并网格同样 ‖K⁻¹R‖≈665）；②装配可重复性（同 u 三次求值逐位一致、沿 Newton 方向残差精确线性）排除索引/状态 bug；③K 健康（SPD、无小对角元）。**真实机理**：Φ 粘结环本征应变失配（相邻匝相容解要求 u_r 差 ε·δr，粘结强迫相等 → 弯曲协调）→ 界面软化 → **Δsoc_n=1.0 在 load_progress≈0.518 真实极限点**（load_substep 停滞警告实测）；与 D13"预应力近临界 μ≈5e-3"独立印证；**Crisfield 弧长由此确认为必需工具**（非可选）。
- **探针 v3 冻结（方案 B，用户选定；`fc0bdf7`）**：默认仅 basic（停滞水平 grinding 分钟级，用户制止了明知停滞的全方法硬跑）；basic 2/8（Δsoc≥1 FAIL 为冻结行为，极限点响应）；load_sub/arc_length 定性"极限点不可越"不纳入逐位比对，`JUBAT_PROBE_ALL=1` 保留三方法路径。网格 Nodes 10144（−802 合并对）、模量与 v2 同。
- **testexample v3 已冻结**：电/热全部指标与 v2 一致；separation 1.2572e-13→6.6820e-15 m（Φ 粘结约束匝间自由度）；PNG SHA `272402bb…`。README/metrics.toml 已更新，v1/v2/v3 档案同存。
- **C4-lite 判定（D10 探针，停止评审不降级）**：6 组合（预应力 0.2/1.0× × Δsoc 0.5/1.0/2.0，nθ=8，geo+塑性+预应力全开）全部收敛，但 **Δ_core≡0.0、D_max≡0.0、κmax 0.0098→0.81 单调发展**。机理：完美对称网格 + 轴对称载荷 → n=0 呼吸响应被 D8 滤波正确滤除；n≥2 需非对称扰动（几何缺陷/局部载荷）激发。多相位测试（Δsoc 0.15→0.20→0.25）同样全零，测试已改为固化 D10 结论（`all(iszero, Δ_hist)` 等断言，绿色）。C4-lite 达成条件留待后续：几何缺陷注入或非对称载荷工况 + Crisfield 弧长修复（Task 2 阻塞联动）。
- **收尾待办**：收尾全套门禁（应 34/34）+ testexample v3 复核 → 提交 `test_czm_multicycle_c4lite.jl`（D10 结论版）+ findings D10 段 → `Simplify/baseline.md` 行 + 本文件/index 状态 → Batch 5 关闭。

### 会话：2026-08-23：提交审查修复启动

- **状态：** 原 Batch 5 收尾暂停；独立修复任务 R0 complete、R1 in progress。
- 修复档案：`docs/planning-with-files/31_堆芯塌陷提交修复/`。
- 已冻结契约：失败子步事务回滚；`phi_pairs` 保留未合并网格真实节点对；`czm_arc_length_alpha` 进入球面约束；字符串能力说明只进入最终结果。
- 修复前失败证据：预应力开启在 `CallModel.jl:151` 精确触发 `Vector{Bool}` 类型错误；非零 `eps_p` 内力为单扣参考值 2 倍；geo 弧长停在 `λ≈0.4000000856`；5 参数 `CzmSubmesh` 构造器缺失；clone 丢失三项拓扑/映射字段。
- C4 对称 D10 在真实提交状态后仍为 `Δ_core≡0,D≡0`，继续作为“未触发损伤”的物理结论，不再表述为联合增长证据。

### 会话：2026-08-23：提交审查修复完成与 Batch 5 收尾

- **状态：** 独立修复 R0–R6 complete；Batch 5 实现和数值收尾门禁完成，C4-lite 物理验收仍未满足。
- 修复完成：预应力生产标记不再触发类型中断；J2 历史只扣除一次且局部不收敛显式失败；geo 弧长使用球面约束、当前状态载荷方向和真实最终残差；旧 `CzmSubmesh` 构造器恢复；真实 `phi_pairs` 与求解网格合并职责分离；C4 测试真实提交跨步状态。
- 门禁：专项无 Broken/Skip；全量 34/34（8m40.2s）；standalone basic 与 v3 逐位一致；testexample 1682/1763、19 步、全部科学指标和 PNG SHA `272402bb…` 一致；两级 `git diff --check` 通过。
- 能力边界不变：受控分离证明损伤状态持久化，但对称 D10 仍 `Δ_core≡0,D≡0`。非对称扰动、SP–涂层接触/摩擦和系统标定必须另行授权与验证。

### 会话：2026-08-23：Batch 6A 尝试回滚

- **用户裁决：** 回滚损伤影响有效电化学面积机制；原因是当前理论影响范围存在疑义且尚未解决。
- **已回滚：** `src/CallModel.jl`、`src/JuBat.jl`、`src/Option.jl`、`src/Parallelsolution.jl`、`src/SPMe.jl` 的本轮修改；删除本轮新增 `test/test_czm_electrochemical_area_feedback.jl`；恢复本轮改动的 spec、`Simplify/baseline.md` 与总索引。
- **保留范围：** 仅在本任务三件套记录回滚原因与重新立项前置问题；此前已提交的 Batch 5/R0–R6 不变，`.mimosa/` 与 `output/testexample/` 不变。
- **当前状态：** Batch 6/7 未开始。重新实施前必须先冻结 `A_eff` 的物理对象、作用方程、守恒边界及与旧面积代理/完全失效/接触热阻链的组合规则。

### 会话：2026-08-24：从机械断点继续

- **用户范围：** 暂不实施损伤对电化学和热模型的影响；从 Batch 5 的 C4-lite 物理验收断点继续。
- **恢复结论：** 旧记录中的 geo 弧长 Task 2 WIP 已由独立修复 R3 完成并通过专项门禁；当前真实断点是对称 D10 未激发 `n≥2` 模态，`Δ_core≡0,D≡0`。
- **本轮边界：** 先审计非轴对称机械扰动的最窄、可验证入口；不接入 `czm_continuous_feedback`，不修改电化学/热残差，不触碰工作区既有 `src/CsvExport.jl` 修改。
- **更早断点（审计发现）：** C4-lite 夹具调用 `ensure_czm_cache` 时沿用了低层默认 `fix_inner=true`，而 `Δ_core` 恰从被固定的第一匝内边界读取；原 D10 的全零结论受错误 BC 污染。先把该验证工况改为显式自由芯部并复跑，暂缓初始几何缺陷实现。
- **BC 修复结果：** 新增 `Γ_in,free` 不受约束的契约测试；自由芯部低载三相位得到递增非零 `Δ_core`，人工非轴对称缺陷不再是当前前置。
- **法向断点与修复：** winding cohesive 固定左法向与 host-inner→host-outer 张开方向相反，导致张开被当成受压。已用失败测试固化并在缓存/非缓存装配统一按 host 拓扑定向；平直条带路径保持原方向。
- **D10 复跑状态：** `Δsoc=0.5/1.0` 四组收敛且最大张开仅达起损阈值的 `0.255–0.433`，`D=0` 合理；`Δsoc=2.0` 两组在约 99.3–99.5% 载荷处停滞并显式失败。待用 geo 弧长对这两组做独立后续，不把失败末态写成零或有效结果。
- **弧长后续：** 已补齐 λ=0 预应力参考态多步 Newton、失败预测子步回滚缩步，并保持最终严格 tol；专项测试绿色。关闭 J2 的高载隔离仍在 `λ≈0.42146` 达到弧长下限并显式停滞，确认剩余断点在几何/CZM 分支校正，非塑性局部积分。
- **停止边界：** 不继续放宽最终残差、J2 局部 tol 或伪造损伤；本轮转入专项/全量/冻结基线门禁与差异审查。C4-lite 联合增长验收保持未满足。
- **最终探针：** `tools/probe_c4lite_free_core.jl` 已按最终源码重跑并写入 `output/probe_c4lite_free_core/probe_results.csv`；四个中载点为有效 `OK`，两个 `Δsoc=2.0` 点显式记录 `ERROR/NA`，未用零值伪装失败态。
- **代码门禁：** mesh 法向、geo 弧长、C4-lite、scale、eigenstrain、耦合装配专项均绿色；`test/runtests.jl` **34/34**（8m43.1s）。本轮保留旧三参数 `compute_separation(elem,node,u)`，新增拓扑有向重载，未打断旧调用接口。
- **冻结门禁：** `testexample` 的电/热/网格/步数/损伤指标不变，但最大法向分离 `6.6820e-15→1.5174e-12 m`、PNG SHA `272402bb…→0946646a…`；standalone v3 由 basic 2/8、全 D=0 变为 4/8，且 `Δsoc=1.5` 出现 `D=0.9890`。两道基线均判 FAIL，确认是法向纠正带来的实质力学迁移而非回归噪声。
- **交付边界：** 不擅自重冻结 v4，不修改 `Simplify/baseline*`，不提交本轮代码；等待用户决定接受正确张开方向的新基线，或回退法向修复以维持 v3。工作区既有 `src/CsvExport.jl` 修改未触碰。
- **用户工况修正：** `Δsoc∈[-1,1]`、`ΔT` 不超过 20°C；本轮按更保守的 `|ΔT|≤20°C` 验证。此前 `Δsoc=1.5/2.0` 的损伤/停滞改列为超范围压力测试，不再作为物理验收断点。
- **物理包络门禁：** 探针已改为 16 个边界/轴点（两个预应力倍率），全部 load-substep 收敛；最大残差 `3.393e-9`、最大起损比 `0.4382`、`D_max=0`、最大 `κ=0.05649`、最大 `Δ_core=0.002755`（约 5.29 μm）。包络内无失败点，因此不触发 `--arc-followup`。
- **更新后的判定：** 正确法向在有效工况内给出有界、方向一致且低于起损阈值的响应；超范围大损伤不阻塞。仍需用户明确接受 v3→v4 的冻结力学输出迁移，之后才可重冻结并独立提交；包络内没有塌陷/损伤联合增长，能力宣称保持克制。

## 5 问重启检查

| 问题 | 回答 |
|---|---|
| 当前在哪里？ | 自由芯部 BC、cohesive 法向和 geo 弧长参考态缺陷已修复；代码门禁 34/34、有效物理包络 16/16，法向漂移已被后续 v4–v8 基线接受；包络内未出现塌陷/损伤，Batch 6/7 未开始 |
| 下一步去哪里？ | 决定接受 host-inner→host-outer 正张开并重冻结 v4，还是回退法向修复保持 v3；不再追逐超范围极限点，Batch 6/7 继续冻结 |
| 目标是什么？ | 补齐工况 C 力学非线性核心，最终实现堆芯塌陷模拟（L4/C4） |
| 已学到什么？ | 见 `findings.md` |
| 已做什么？ | 完成代码修复、生产契约测试、全量/standalone/testexample 门禁与治理文档同步；见本文件上述记录 |
